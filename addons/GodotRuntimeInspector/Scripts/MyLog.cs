namespace GodotRuntimeInspector.Scripts
{
    public class MyLog
    {
        ...
        private void WriteTest()
        {
            ...
            txt += nameof(ReadCounter) + " " + ReadCounter + " ";
            txt += nameof(WriteCounter) + " " + WriteCounter + " ";
            Godot.GD.Print(System.DateTime.Now.ToLongTimeString() + " " + txt);
        }

        public void Update(double totalDelta)
        {
            uint roundedTime = (uint)System.Math.Round(totalDelta);
            bool read = roundedTime % SecondsPerRead == 0;
            TimeSinceLastLogRead = totalDelta - LastLogRead;
            if (read == true && TimeSinceLastLogRead > SecondsPerRead)
            {
                LastLogRead = roundedTime;
                GodotRuntimeInspector.MyLog.ReadFile();
            }
        }
    }
}
